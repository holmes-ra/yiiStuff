...
/**
 * This is the model class for table "yapeal_charContactList".
 *
 * The followings are the available columns in table 'yapeal_charContactList':
 ...
 */
class YCharContactList extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return YCharContactList the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			array('ownerID, contactID, contactName, inWatchlist, standing', 'required'),
			array('inWatchlist', 'numerical', 'integerOnly'=>true),
			array('ownerID, contactID', 'length', 'max'=>20),
			array('contactName', 'length', 'max'=>255),
			array('standing', 'length', 'max'=>5),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('ownerID, contactID, contactName, inWatchlist, standing', 'safe', 'on'=>'search'),
		);
	}
