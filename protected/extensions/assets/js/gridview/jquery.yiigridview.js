	...
	methods = {
		...
		init: function (options) {
			...
			return this.each(function () {
				...
				/**
				 * Modified to support 'disabled' class, which will not select rows / checkmarks - Ryan Holmes
				 */
				if (settings.selectableRows > 0) {
					...
					$(document).on('click.yiiGridView', '#' + id + ' table > tbody > tr', function (e) {	
						...
						if ($target.closest('td').is('.empty,.button-column') || (e.target.type === 'checkbox' && !$target.hasClass('select-on-check')) || $(this).hasClass('disabled') ) {
							return;
						}
					...
					if (settings.selectableRows > 1) {
						$(document).on('click.yiiGridView', '#' + id + ' .select-on-check-all', function () {
							var $currentGrid = $('#' + id),
								$checks = $('input.select-on-check', $currentGrid).not(':disabled'),
								$checksAll = $('input.select-on-check-all', $currentGrid).not(':disabled'),
								$rows = $currentGrid.children('.' + settings.tableClass).children('tbody').children().not('.disabled');
							if (this.checked) {
